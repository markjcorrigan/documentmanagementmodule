                    ...
                    <div class="hidden md:block">
                        <table class="table table-striped table-bordered w-full border-collapse border border-gray-400">
                            ...
                            @foreach($documents as $doc)
                                <tr>
                                    <td class="border border-gray-400 px-4 py-2">{{ $doc->id }}</td>
                                    <td class="border border-gray-400 px-4 py-2">{{ $doc->name }}</td>
                                    <td class="border border-gray-400 px-4 py-2">{{ $doc->shortname }}</td>
                                    <td class="border border-gray-400 px-4 py-2">{{ $doc->path }}</td>
                                    <td class="border border-gray-400 px-4 py-2">{{ $doc->created_at }}</td>
                                    <td class="border border-gray-400 px-4 py-2">{{ $doc->updated_at }}</td>
                                    <td class="border border-gray-400 px-4 py-2 flex flex-wrap gap-2">
                                        <a wire:click.prevent="editDocument({{ $doc->id }})" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded text-center">Edit</a>
                                        <button wire:click="deleteDocument({{ $doc->id }})" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded">Delete</button>
                                        <a href="{{ route('downloadByShortName', $doc->shortname) }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-2 rounded text-center">Download</a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
                        ...
                        <!-- Pagination -->
                        <div class="mt-4">
                            {{ $documents->links() }}  <!-- Make sure Livewire pagination is triggered -->
                        </div>
                    </div>
                @else
