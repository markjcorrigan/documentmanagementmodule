...
@section('content')
                        ...
                        <form action="{{ route('jokes.update', $joke) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="title" id="title" rows="5"
                                          ...
                                          required>{{ old('title', $joke->title) }}</textarea>
                                @error('title')
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="description" id="description" rows="3"
                                          class="form-control @error('description') is-invalid @enderror">{{ old('description', $joke->description) }}</textarea>
                                @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="jokecat_id" id="jokecat_id"
                                    ...
                                    @foreach($jokecats as $cat)
                                        <option value="{{ $cat->id }}"
                                                {{ old('jokecat_id', $joke->jokecat_id) == $cat->id ? 'selected' : '' }}>
                                            {{ $cat->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <button type="submit" class="btn btn-primary">
                                    <i class="bi bi-check-circle"></i> Update Joke
                                </button>
                            </div>
                        </form>
                    </div>
    ...
    @else
        <div class="alert alert-danger">
            <h4>Access Denied</h4>
            <p>You do not have permission to edit jokes.</p>
            <a href="{{ route('jokes.index') }}" class="btn btn-secondary">
