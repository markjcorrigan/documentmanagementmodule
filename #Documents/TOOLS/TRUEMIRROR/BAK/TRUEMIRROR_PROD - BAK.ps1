...
if ($operationMode -eq "FULL_MIRROR") {
    ...
    Log-Header "INCREMENTAL UPDATE: ANALYZING CHANGES"
    Log "Scanning source and destination to identify changes..." "Cyan"
    
    # Get incremental changes
    $changes = Get-IncrementalChanges -Source $source -Destination $destination
    
    Log "Incremental analysis complete:" "Green"
    Log "  New files to copy: $($changes.ToCopy.Count)" "Cyan"
    Log "  Changed files to update: $($changes.ToUpdate.Count)" "Cyan"
    Log "  Files to delete (no longer in source): $($changes.ToDelete.Count)" "Cyan"
    Log "  Total source files: $($changes.SourceFileCount)" "Gray"
    Log "  Total destination files: $($changes.DestFileCount)" "Gray"
    
    if ($changes.ToCopy.Count -eq 0 -and $changes.ToUpdate.Count -eq 0 -and $changes.ToDelete.Count -eq 0) {
        Log "No changes detected - destinations are already in sync!" "Green"
        Log "Exiting incremental update." "Green"
        Write-Host "`nPress ENTER to exit..."
        Read-Host
        exit 0
    }
...
# ===================== COPY FILES =====================
if ($operationMode -eq "FULL_MIRROR") {
    Log-Header "COPYING AND VERIFYING FILES (FULL MIRROR)"
    ...
    Log-Header "COPYING AND VERIFYING FILES (INCREMENTAL UPDATE)"
    
    # Combine new and changed files
    $allFilesToProcess = $changes.ToCopy + $changes.ToUpdate
    $totalFiles = $allFilesToProcess.Count
    
    if ($totalFiles -eq 0) {
        Log "No files to copy or update." "Green"
        $filesToProcess = @()
        $processAll = $false
    } else {
        Log "Processing $totalFiles file(s) (new: $($changes.ToCopy.Count), changed: $($changes.ToUpdate.Count))" "Cyan"
        
        # Convert relative paths to full paths
        $filesToProcess = @()
        foreach ($relativePath in $allFilesToProcess) {
            $fullPath = Join-Path $source $relativePath
            if (Test-Path $fullPath) {
                $filesToProcess += Get-Item -LiteralPath $fullPath -Force
            }
        }
        $processAll = $true
    }
...
if ($processAll -and $filesToProcess.Count -gt 0) {
    $copied = 0
    $failed = @()
    $skipped = 0
    $longPathUsed = 0
