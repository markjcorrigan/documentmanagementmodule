...
- Backup logs in BOTH source and destination directories

How to run:
1. Open PowerShell 7+ as Administrator
2. Run: .\TRUEMIRROR_ROOTBACKUP.ps1
3. Log file: C:\enhanced_mirror.log
4. Source backup log: C:\xampp\htdocs\pmway.hopto.org\backup.log
5. Destination backup log: C:\pmway.hopto.org\backup.log
#>

# ===================== CONFIG =====================
$source = "C:\xampp\htdocs\pmway.hopto.org"        # Source folder
$destination = "C:\pmway.hopto.org"   # Destination folder
$logFile = "C:\pmway.hopto.org\enhanced_mirror.log"  # Your destination drive
...
# ===================== DRIVE CONFIGURATION =====================
$destinationDrive = "C"  # Destination drive letter
...
# ===================== SOURCE BACKUP LOG =====================
$sourceBackupLogFile = Join-Path $source "backup.log"
...
- New files copied: $($changes.ToCopy.Count)
- Changed files updated: $($changes.ToUpdate.Count)
- Obsolete files deleted: $deletedCount
"@
        if ($deleteFailed.Count -gt 0) {
            $sourceBackupEntry += "`n- Failed deletions: $($deleteFailed.Count)"
        }
    } else {
        $sourceBackupEntry += "`n- Files deleted from destination: $deletedCount"
    }
...
- Hash result: $(if($runFinalHashCheck){if($sourceHash -eq $destHash){'PASSED - Folders identical'}else{'FAILED - Differences detected'}}else{'N/A - Skipped for speed'})

STATUS: $(if($failed.Count -eq 0){'✅ SUCCESS'}else{"⚠️  COMPLETED WITH $($failed.Count) ERROR(S)"})
...
"@
    ...
    if ($failed.Count -gt 0) {
        $sourceBackupEntry += "`nFAILED FILES (first 10):`n"
        $failed | Select-Object -First 10 | ForEach-Object { 
            $sourceBackupEntry += "  - $_`n" 
        }
        if ($failed.Count -gt 10) {
            $sourceBackupEntry += "  ... and $($failed.Count - 10) more (see main log)`n"
        }
    }

    # Add large files summary if any
    if ($largeFilesProcessed.Count -gt 0) {
        $sourceBackupEntry += "`nLARGE FILES PROCESSED (>= $largeFileSizeMB MB):`n"
        $largeFilesSorted = $largeFilesProcessed | Sort-Object -Property SizeMB -Descending | Select-Object -First 5
        foreach ($largeFile in $largeFilesSorted) {
            $sourceBackupEntry += "  $($largeFile.SizeMB) MB - $($largeFile.Path)`n"
        }
        if ($largeFilesProcessed.Count -gt 5) {
            $sourceBackupEntry += "  ... and $($largeFilesProcessed.Count - 5) more`n"
        }
        $totalLargeSize = ($largeFilesProcessed | Measure-Object -Property SizeMB -Sum).Sum
        $sourceBackupEntry += "  Total: $([math]::Round($totalLargeSize / 1024, 2)) GB`n"
    }

    $sourceBackupEntry += "`n"

    # Append to existing log file or create new one
    if (Test-Path $sourceBackupLogFile) {
        $sourceBackupEntry | Out-File -FilePath $sourceBackupLogFile -Append -Encoding UTF8
        Log "✓ Backup history appended to SOURCE: $sourceBackupLogFile" "Green"
    } else {
        # Create new log with header
        $logHeader = @"
===========================================================
SOURCE BACKUP HISTORY LOG
===========================================================
Source folder: $source
This log tracks all backup operations FROM this source folder.
Created: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')
===========================================================

"@
        $logHeader + $sourceBackupEntry | Out-File -FilePath $sourceBackupLogFile -Force -Encoding UTF8
        Log "✓ Created SOURCE backup log: $sourceBackupLogFile" "Green"
    }
    
    # Also create a simple timestamp file for quick reference
    $sourceTimestampFile = Join-Path $source "last_backup.txt"
    $timestampContent = @"
Last backup: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')
Mode: $operationMode
Destination: $destination
Files: $($allSourceFiles.Count)
Status: $(if($failed.Count -eq 0){'SUCCESS'}else{"$($failed.Count) ERRORS"})
"@
    $timestampContent | Out-File -FilePath $sourceTimestampFile -Force
    
} catch {
    Log "Warning: Could not create SOURCE backup log file: $($_.Exception.Message)" "Yellow"
}

# ===================== DESTINATION BACKUP LOG =====================
$destBackupLogFile = Join-Path $destination "backup.log"
...
$separator
BACKUP RECEIVED - $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')
$separator
...
- New files received: $($changes.ToCopy.Count)
- Changed files updated: $($changes.ToUpdate.Count)
- Obsolete files removed: $deletedCount
"@
        if ($deleteFailed.Count -gt 0) {
            $destBackupEntry += "`n- Failed deletions: $($deleteFailed.Count)"
        }
    } else {
        $destBackupEntry += "`n- Old files cleared: $deletedCount"
    }
...
VERIFICATION:
- Hash check performed: $(if($runFinalHashCheck){'Yes'}else{'No'})
- Hash result: $(if($runFinalHashCheck){if($sourceHash -eq $destHash){'PASSED - Mirror verified'}else{'FAILED - Differences exist'}}else{'N/A - Skipped'})
...
"@
    ...
    # Append to existing log file or create new one
    if (Test-Path $destBackupLogFile) {
        $destBackupEntry | Out-File -FilePath $destBackupLogFile -Append -Encoding UTF8
        Log "✓ Backup history appended to DESTINATION: $destBackupLogFile" "Green"
    } else {
        # Create new log with header
        $logHeader = @"
===========================================================
DESTINATION BACKUP HISTORY LOG
===========================================================
Destination folder: $destination
This log tracks all backup operations TO this destination folder.
Created: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')
===========================================================

"@
        $logHeader + $destBackupEntry | Out-File -FilePath $destBackupLogFile -Force -Encoding UTF8
        Log "✓ Created DESTINATION backup log: $destBackupLogFile" "Green"
    }
    
    # Also create a simple timestamp file for quick reference
    $destTimestampFile = Join-Path $destination "last_backup.txt"
    $timestampContent = @"
Last backup: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')
Mode: $operationMode
Source: $source
Files: $($allDestFiles.Count)
Status: $(if($failed.Count -eq 0){'SUCCESS'}else{"$($failed.Count) ERRORS"})
"@
    $timestampContent | Out-File -FilePath $destTimestampFile -Force
    
} catch {
    Log "Warning: Could not create DESTINATION backup log file: $($_.Exception.Message)" "Yellow"
}
